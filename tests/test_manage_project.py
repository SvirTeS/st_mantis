...
project_data = Project(name=random_string('name', 10), description=random_string('header', 20))


def test_add_project(app):
    app.session.ensure_login('administrator', 'root')
    old_list_project = app.soap.get_project_list_from_soap()
    app.project.create(project_data)
    new_list_project = app.soap.get_project_list_from_soap()
...
def test_del_random_project_by_id(app):
    app.session.ensure_login('administrator', 'root')
    if len(app.soap.get_project_list_from_soap()) == 0:
        app.project.create(project_data)
    old_list_project = app.soap.get_project_list_from_soap()
